...
if [ "$PANE_CMD" = "ssh" ] && [ "$SESSION_NAME" != "claude" ]; then
  ...
  # Remote Claude connects directly to local MCP server via the reverse tunnel
  if [ -n "$MCP_PORT" ]; then
  ...
  if [ -n "$REMOTE_DIR" ]; then
    ...
    REMOTE_CMD="ssh -t $MCP_TUNNEL_FLAGS '$SSH_HOST' 'zsh -lic \"${MCP_NOTIFY_SETUP}${REMOTE_MCP_ENV:+$REMOTE_MCP_ENV }claude $FLAGS${MCP_NOTIFY_CLEANUP}\"'"
  fi
