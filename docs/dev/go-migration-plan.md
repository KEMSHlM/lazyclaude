...
| # | タスク | 成果物 |
|---|--------|--------|
| P4.1 | Session 型 + Store (JSON 永続化) | `internal/session/store.go` |
| P4.2 | SessionManager (CRUD + tmux 同期) | `internal/session/manager.go` |
| P4.3 | SessionListContext (リスト表示 + カーソル) | `internal/gui/context/sessions.go` |
| P4.4 | SessionController (n/d/enter/r/R) | `internal/gui/controllers/session.go` |
| P4.5 | セッション行フォーマット | `internal/gui/presentation/sessions.go` |
| P4.6 | PreviewContext (capture-pane ポーリング) | `internal/gui/context/preview.go` |
| P4.7 | launch (local) | `internal/session/launch.go` |
| P4.8 | attach ループ | `internal/session/attach.go` |
...
| # | タスク | 成果物 |
|---|--------|--------|
| P5.1 | DiffContext (diff パース + chroma ハイライト) | `internal/gui/context/diff.go` |
| P5.2 | DiffController (スクロール + y/a/n) | `internal/gui/controllers/diff.go` |
| P5.3 | diff 行カラーリング | `internal/gui/presentation/diff.go` |
| P5.4 | ToolContext (ツール情報表示) | `internal/gui/context/tool.go` |
| P5.5 | ToolController (y/a/n) | `internal/gui/controllers/tool.go` |
| P5.6 | choice ファイル書き出し | `internal/gui/context/choice.go` |
| P5.7 | MCP サーバーとの接続 (P2 と結合) | `internal/server/popup.go` 更新 |
...
| M5.5 | TUI 自動テスト | P5.5 | termtest + tmux E2E + VHS が CI で動作 | **testing** |
