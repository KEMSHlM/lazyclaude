...
### Step 6: Render layer の placeholder 対応

Plugin panel / MCP panel の render 関数で `pluginState.remoteDisabled` / `mcpState.remoteDisabled` を読み、true なら通常のリスト描画を skip して placeholder を出す。

ファイル候補:
- `internal/gui/render_plugins.go` (Plugin panel render)
- MCP panel render (ファイル位置は worker が確認)

Placeholder 例:
```
[Remote session]
Plugin editing on remote hosts is not
supported in this build.

Switch cursor to a local session to
manage plugins.
```

同等の文言を MCP panel にも。

Render 関数内の cursor 移動や空リスト時の既存処理を壊さないように worker が慎重に追加。
...
2. `syncPluginProjectOnce`:
   - 初回 startup で remote node が選択されている場合: fallback の CWD Refresh が **呼ばれないこと**
   - Session tree 空で fallback を通る場合: 従来通り動く
3. Write guards:
...
Mock は既存の `mockActions` (keydispatch) / `mockFullScreenActions` (keyhandler) や test 内 mock provider を活用。既存 test 内で plugin/mcp mock が既に存在するなら流用。
...
| `internal/gui/render_plugins.go` (+ MCP panel render) | `remoteDisabled` 時の placeholder 表示 |
